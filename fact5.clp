...
(defrule calculate-price
   (exists (UserSelection))
   =>
   (bind ?total-count 0) ; Initialize total count of pizzas

   ;; Calculate total count of pizzas
   (foreach (?selection <- (UserSelection)) do
     (bind ?total-count (+ ?total-count (fact-slot-value ?selection count)))
   )

   ;; Check if the total count of pizzas is divisible by 4
   (if (= (mod ?total-count 4) 0)
       then
       ;; Apply discount if divisible by 4
       (foreach (?selection <- (UserSelection)) do
         (bind ?drink_price (fact-slot-value (c-lookup (c-lookup ?selection pizza) size) price)) ; Get drink price
         (bind ?drink_total_price (* ?drink_price (fact-slot-value ?selection drink_count))) ; Calculate total price for drinks for the selected pizza
         (bind ?pizza_price (fact-slot-value ?selection price)) ; Get pizza price
         (bind ?base-price (+ ?drink_total_price (* ?pizza_price (fact-slot-value ?selection count)))) ; Calculate base price without discount
         (bind ?discounted-price (- ?base-price (* 0.1 ?base-price))) ; Calculate discounted price
         (assert (Result
                  (drink (fact-slot-value ?selection drink))
                  (drink_count (fact-slot-value ?selection drink_count))
                  (pizza (fact-slot-value ?selection pizza))
                  (size (fact-slot-value ?selection size))
                  (count (fact-slot-value ?selection count))
                  (total-price ?discounted-price)))
       )
       else
       ;; No discount applied if not divisible by 4
       (foreach (?selection <- (UserSelection)) do
         (bind ?drink_price (fact-slot-value (c-lookup (c-lookup ?selection pizza) size) price)) ; Get drink price
         (bind ?drink_total_price (* ?drink_price (fact-slot-value ?selection drink_count))) ; Calculate total price for drinks for the selected pizza
         (bind ?pizza_price (fact-slot-value ?selection price)) ; Get pizza price
